...
private fun renameVariablesUsingMarkerVariables(node: MethodNode, scopeOffset: Int): Int {
    ...
    for (variable in sortedVariables) {
        ...
        if (isFakeLocalVariableForInline(name)) {
            previousScopeNumber = name.scopeNumber
            lastInlineScopeIndex += 1
            currentInlineScopeIndex = lastInlineScopeIndex

            val newName = newName(variable)
            variable.name = if (name.startsWith(JvmAbi.LOCAL_VARIABLE_NAME_PREFIX_INLINE_ARGUMENT)) {
                val surroundingScopeNumber = name.surroundingScopeNumber
                // The lambda that we are inspecting the marker variable for is being inlined somewhere
                if (currentInlineScopeIndex == 1) {
                    newName.addScopeNumber(0)
                }
                // Every lambda that is already inlined must have a surrounding scope number.
                // If it does not, then it means that we are inlining the code compiled by
                // older versions of the Kotlin compiler, when surrounding scope numbers
                // haven't been introduced yet.
                else if (surroundingScopeNumber != null) {
                    if (surroundingScopeNumber == 0) {
                        newName.addScopeNumber(scopeOffset + 1)
                    } else {
                        newName.addScopeNumber(surroundingScopeNumber + scopeOffset + 1)
                    }
                }
                // Inlining from the old code
                else {
                    val depth = getInlineDepth(name)
                    val scopeNumber = inlineScopes.getOrNull(depth)?.second?.plus(scopeOffset) ?: continue
                    newName.addScopeNumber(scopeNumber)
                }
//                val surroundingScope = inlineScopes.firstOrNull { it.name.startsWith(JvmAbi.LOCAL_VARIABLE_NAME_PREFIX_INLINE_ARGUMENT) }
//                if (surroundingScope != null) {
//                    val surroundingScopeNumber = surroundingScope.name.scopeNumber!!
//                    newName.addScopeNumber(surroundingScopeNumber)
//                } else {
//                    newName
//                }
            } else {
    ...
    return currentInlineScopeIndex
}

private fun renameVariablesUsingScopeNumbers(node: MethodNode, scopeOffset: Int): Int {
    val inlineScopesSet = mutableSetOf<Int>()
    val oldToNewScopeNumbers = mutableMapOf<Int, Int>()
    for (variable in node.localVariables) {
        val scopeNumber = variable.name
            .withoutInlineVarSuffix()
            .scopeNumber
        if (scopeNumber != null) {
    ...
    for (variable in node.localVariables) {
        ...
        if (scopeNumber != null) {
            oldToNewScopeNumbers[scopeNumber] = newScopeNumber
        }

        variable.name = name.withoutScopeNumber().addScopeNumber(newScopeNumber)
    }

    return inlineScopesSet.size + 1
}

private const val INLINE_SCOPE_NUMBER_SEPARATOR = '\\'

private fun String.withoutScopeNumber(): String =
    substringBefore(INLINE_SCOPE_NUMBER_SEPARATOR)

private val String.scopeNumber: Int?
    get() = substringAfter(INLINE_SCOPE_NUMBER_SEPARATOR).substringBefore(INLINE_SCOPE_NUMBER_SEPARATOR).toIntOrNull()

private val String.surroundingScopeNumber: Int?
    get() = substringAfter(INLINE_SCOPE_NUMBER_SEPARATOR, "").substringAfter(INLINE_SCOPE_NUMBER_SEPARATOR, "").toIntOrNull()
